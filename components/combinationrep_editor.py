...
def combinationrep_editor():
    ...
    if cols[2].button('Calcular Combinación',use_container_width=True):
        omega = set(itertools.combinations_with_replacement(combinatorial_space, glen))
        st.write(omega)
        st.write(f'Número de elementos en $\Omega$: {len(omega)}')

    if cols[1].button('Crear Espacio Muestral',use_container_width=True):
        st.session_state.omega = set(itertools.combinations_with_replacement(combinatorial_space, glen))
