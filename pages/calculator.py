...
def workspace_view():
    with st.container(border=True):
        ...
        st.write(
            "Eventos en el espacio de trabajo:", str(st.session_state.events_workspace)
        )
...
def probability_calculator_conditional(main):
    ...
    if main.button("Calcular Probabilidad Condicional"):
        ...
        st.write(
            f"$P({A}|{B}) = {len(st.session_state.events[A].intersection(st.session_state.events[B]))}/{len(st.session_state.events[B])}$"
        )
...
def probability_calculator(main):
    ...
    if main.button("Calcular Probabilidad"):
        ...
        st.write(
            f"$P({A}) = {len(st.session_state.events[A])}/{len(st.session_state.omega)}$"
        )
...
if __name__ == "__main__":
    ...
    if lateral.toggle(
        "Añadir Evento al espacio de trabajo",
    ):
